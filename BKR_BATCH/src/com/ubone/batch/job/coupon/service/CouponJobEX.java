 ...
 */
@Service
public class CouponJobEX {
	
	@Resource
	private CouponJobDAO couponJobDAO;
	
	Log logger = LogFactory.getLog(CouponJobEX.class);
				...
				for (int j=0; j<valFirstPin; j++) {
				    ...
				    if (nl != null && nl.getLength() > 0 && "S000001".equals(rt)) {
			            if (nl.item(0).getNodeType() == Node.ELEMENT_NODE) {
			                ...
			                insertParam.setParameter("DT_EXPIRY_START", dtData.getString(i, "DT_PIN_START"));
			                insertParam.setParameter("DT_EXPIRY_END", dtData.getString(i, "DT_PIN_END"));
	...
	private Document getCouponPinXmlDocumentFromDoau(Parameter parameter, String gubun) {
		...
		try {
			...
			if ("S".equals(gubun)) {	// 실시간 요청일경우 저장되어 있는 데이터 사용
				cooperOrder = parameter.getParameter("COOPER_ORDER").toString();
			}
